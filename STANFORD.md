...
- **For the Plus:** runs every `agent-*` service (Dodo :2339, Bridge, diag,
  ...
  modem all find the mini at **192.168.1.50**, hard-coded in the apps, on the
...
1. **Travel router** (GL.iNet or a spare home router). Wi-Fi = the exact home
   SSID/password; LAN = 192.168.1.0/24; DHCP reservation mini → 192.168.1.50.
...
(`DODO_IP` etc. — grep `192.168.1.50` in apps/macplus).
