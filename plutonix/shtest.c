...
int main(void) {
    ...
    ok("mini targets rsh :2329", gRemPortT==2329 && strstr(gRemHostT,"192.168.1.50")!=0);
    rc=run("rsh");   ok("rsh -> rc 4", rc==4);
    ok("bare rsh targets the mini :2329", gRemPortT==2329 && strstr(gRemHostT,"192.168.1.50")!=0);
    rc=run("rsh 192.168.1.99:2000"); ok("rsh host:port -> rc 4", rc==4);
    ok("rsh host:port parsed", gRemPortT==2000 && !strcmp(gRemHostT,"192.168.1.99"));
    rc=run("rsh 192.168.1.99"); ok("rsh host default port", rc==4 && gRemPortT==2329 && !strcmp(gRemHostT,"192.168.1.99"));
