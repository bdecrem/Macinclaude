...
2. **`vmodem.py`** — a virtual Hayes modem. Listens on `127.0.0.1:5454`; speaks
   ...
   `ATDT"192.168.1.50:2324"`) reaches the mini.
...
2. **Settings**: Modem port, **9600** baud, host **192.168.1.50**, port
...
3. The app dials. Watch `/tmp/vmodem.log`: you'll see `AT`, then
   `ATDT"192.168.1.50:2324"`, then `CONNECT`, then the live agent session
