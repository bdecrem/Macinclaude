...
```bash
ssh admin@192.168.1.50 'bash ~/macinclaude/backend/update.sh'
# one-time, the first time only:
ssh admin@192.168.1.50 'cd ~/macinclaude/agent-porthole && npx playwright install chromium'
ssh admin@192.168.1.50 'bash ~/macinclaude/backend/install-agents.sh porthole'
```
