...
**deploy clone** `~/macinclaude` as LaunchAgent **`sh.macplus.code`** on port 2324 (2323 stays the
plain login shell). **There is no rsync and no `~/claude-plus` copy anymore.** To deploy: push to
`main`, then run `bash ~/macinclaude/backend/update.sh` on the mini — code-only changes
